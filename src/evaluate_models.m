...
for fold = 1:k
    ...
    rows = [rows; metricRow(fold, models(1), yTest, predLogistic, ...
        positiveScore(scoreLogistic, logistic.ClassNames))]; %#ok<AGROW>
    ...
    rows = [rows; metricRow(fold, models(2), yTest, predForest, ...
        scoreForest(:, positiveColumn))]; %#ok<AGROW>
end

foldMetrics = struct2table(rows);
summary = groupsummary(foldMetrics, "model", ["mean", "std"], cellstr(metricNames));

cohortSummary = table(height(dataset), numel(unique(dataset.subject_id)), ...
    sum(dataset.sepsis_label == 1), sum(dataset.sepsis_label == 0), ...
    "VariableNames", ["admissions", "patients", "sepsis_admissions", ...
    "nonsepsis_admissions"]);

results = struct("foldMetrics", foldMetrics, "summary", summary, ...
    "cohortSummary", cohortSummary, "featureNames", cfg.featureNames);
end
